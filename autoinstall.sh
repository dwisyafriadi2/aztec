...
function install_docker() {
  if command -v docker &> /dev/null; then
    echo -e "${LIGHT_GREEN}Docker sudah terpasang, versi: $(docker --version)${RESET}"
    return
  fi
...
# Fungsi untuk uninstall Aztec Node
function uninstall_aztec_node() {
  echo -e "${RED}Menghentikan Aztec Node...${RESET}"
  PID=$(pgrep -f start_aztec_node.sh)
  if [[ -n "$PID" ]]; then
    kill $PID && echo -e "${GREEN}Aztec Node dihentikan (PID: $PID).${RESET}"
  else
    echo -e "${YELLOW}Tidak ada proses Aztec Node yang berjalan.${RESET}"
  fi

  read -p "Apakah Anda ingin menghapus data Aztec (y/n)? " RESP
  if [[ "$RESP" =~ ^[Yy]$ ]]; then
    rm -rf ~/.aztec
    echo -e "${GREEN}Data Aztec dihapus.${RESET}"
  fi
  ...
  do
      case $opt in
          ...
          "Masuk ke Screen untuk Cek Logs")
              ...
              break
              ;;
          "Uninstall Aztec Node")
              uninstall_aztec_node
              break
              ;;
          "Keluar")
